...
interface Product { id: number; name: string; price: number; categoryId: number; stock: number; }
interface OrderItem { id: number; quantity: number; total: number; Product: Product; }
...
const Sales: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        setCategories(res.data.filter((c: Category) => c.name !== 'Matéria-Prima'));
  ...
  const handleAddItem = async () => {
    ...
    try {
      // 1️⃣ Buscar pedidos existentes
      const existingOrdersRes = await axios.get('http://localhost:5000/orders');
      const existingOrders = existingOrdersRes.data;
      const existingOrder = existingOrders.find((o: any) => o.tableNumber.toLowerCase() === tableNumber.toLowerCase());

      const itemTotal = selectedProductPrice * quantity;

      if (existingOrder) {
        // Adicionar item à comanda existente
        const res = await axios.post(`http://localhost:5000/orders/${existingOrder.id}/items`, {
        ...
        if (res.data.orderClosed) navigate('/Menu');
      } else {
        // Criar nova comanda
        await axios.post('http://localhost:5000/orders', {
          tableNumber,
          paymentMethod: 'dinheiro',
          items: [{ productId: Number(selectedProduct), quantity, total: itemTotal, note }],
        });
      }
      ...
      if (['porção', 'lanche'].includes(selectedCategory.toLowerCase())) {
        printJS({
          printable: [{ productName: selectedProductName, quantity, price: selectedProductPrice, total: itemTotal, note }],
      ...
      console.error(error);
      showError(error.response?.data?.error || 'Erro ao adicionar item.');
    }
  };

  return (
    <div className="sales-container">
      <Link to="/Menu"><img src={logo} alt="Logo" className="sales-logo" /></Link>
      <h2 className="centered-title">VENDAS</h2>

      <label>Mesa / Cliente:
        <input type="text" value={tableNumber} onChange={(e) => setTableNumber(e.target.value)} placeholder="Informe a mesa" />
      </label>
      ...
      <label>Observação:
        <input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Ex: sem sal, sem gelo..." />
      </label>

      <label>Quantidade:
        <input type="number" min={1} max={selectedProductStock} value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} />
      </label>
